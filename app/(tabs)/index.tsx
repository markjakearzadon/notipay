...
import { apiClient, PaymentNotice, mapStatusToDisplay } from '../api';
...
const PaymentList = () => {
  ...
  const loadPaymentRequests = async () => {
    try {
      ...
      const result = await apiClient.getMyPaymentRequests(token);
      
      if (result.success) {
        setPaymentRequests(result.data || []);
      } else {
        Alert.alert('Error', result.error || 'Failed to load payment requests');
      }
    } catch (error) {
  ...
  const handlePay = async (notice: PaymentNotice) => {
    if (notice.status === 1) { // 1 = Paid
      Alert.alert('Already Paid', 'This payment has already been completed.');
      return;
    }
            ...
            {paymentRequests.map((item) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.paymentCard,
                    item.status === 1 && styles.paidCard // Special style for paid
                  ]}
                  onPress={() => handlePay(item)}
                  activeOpacity={item.status === 1 ? 1 : 0.95}
